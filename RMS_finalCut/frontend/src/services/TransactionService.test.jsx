...
const API_BASE_URL = "https://localhost:5003/api/transactions";
...
describe("TransactionService API Tests", () => {
  // fetchTransactions
  test("fetchTransactions should return an empty array if role or ID is missing", async () => {
    const transactions = await TransactionService.fetchTransactions("", "");
    expect(transactions).toEqual([]);
  });

  test("fetchTransactions should handle network errors gracefully", async () => {
    mock.onGet(`${API_BASE_URL}/user/artist/1`).networkError();
    const transactions = await TransactionService.fetchTransactions("Artist", 1);
    expect(transactions).toEqual([]);
  });

  test("fetchTransactions should return transactions for a valid role and ID", async () => {
  ...
  test("fetchTransactions should handle invalid role gracefully", async () => {
  ...
  test("fetchTransactions should handle server errors gracefully", async () => {
  ...
  // fetchWalletAmount
  test("fetchWalletAmount should calculate wallet amount for Artist role", () => {
    const transactions = [
  ...
  test("fetchWalletAmount should calculate wallet amount for Manager role", () => {
    const transactions = [
      { id: 1, artistShare: 100, managerShare: 50 },
      { id: 2, artistShare: 200, managerShare: 100 },
    ];
...
//   test("fetchWalletAmount should handle null or undefined transactions", () => {
//     const total = TransactionService.fetchWalletAmount(null, "Artist");
//     expect(total).toBe(0);
//   });

  test("fetchWalletAmount should handle transactions with null or undefined values", () => {
    const transactions = [
  ...
  test("downloadTransactionsPDF should return an error if role or ID is missing", async () => {
    await expect(TransactionService.downloadTransactionsPDF("", ""))
      .rejects.toThrow("Failed to download PDF");
  });

  test("downloadTransactionsPDF should handle network errors gracefully", async () => {
    mock.onGet(`${API_BASE_URL}/export/artist/1`).networkError();
    await expect(TransactionService.downloadTransactionsPDF("Artist", 1))
      .rejects.toThrow("Failed to download PDF");
  });

  test("downloadTransactionsPDF should download PDF for a valid role and ID", async () => {
  ...
  test("downloadTransactionsPDF should handle server errors gracefully", async () => {
  ...
  test("downloadTransactionsPDF should handle invalid role gracefully", async () => {
    await expect(TransactionService.downloadTransactionsPDF("Invalid", 1))
      .rejects.toThrow("Failed to download PDF");
  });

  // createTransaction
  test("createTransaction should return an error when transaction data is invalid", async () => {
  ...
  test("createTransaction should create a transaction with valid data", async () => {
  ...
  test("createTransaction should handle missing required fields", async () => {
  ...
  test("createTransaction should handle server errors gracefully", async () => {
    const transactionData = { amount: 100, description: "Test transaction" };
  ...
  // deleteTransaction
  test("deleteTransaction should return an error when transaction ID is invalid", async () => {
  ...
  test("deleteTransaction should delete a transaction with a valid ID", async () => {
  ...
  test("deleteTransaction should handle server errors gracefully", async () => {
  ...
  // payTransaction
  test("payTransaction should return an error when transaction ID or payment amount is missing", async () => {
  ...
  test("payTransaction should return an error when API responds with failure", async () => {
  ...
  test("payTransaction should process payment with valid ID and amount", async () => {
  ...
  test("payTransaction should handle server errors gracefully", async () => {
