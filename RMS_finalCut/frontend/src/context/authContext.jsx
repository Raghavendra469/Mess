...
export const AuthProvider = ({ children }) => {
    ...
    const verifyUser = async () => {
        ...
        if (token) {
            try {
                const response = await axios.get("http://3.223.75.62:5001/api/auth/verify", {
                    headers: {
    ...
    const fetchRoleData = async (loggedInUser) => {
        try {
            const response = await axios.get(`http://3.223.75.62:5005/api/users/${loggedInUser.username}`,{
                headers: {
                    "Content-Type": "application/json",
