...
export const AuthProvider = ({ children }) => {
    ...
    const verifyUser = async () => {
        ...
        if (token) {
            try {
                const response = await axios.get("http://54.163.10.39:5001/api/auth/verify", {
                    headers: {
    ...
    const fetchRoleData = async (loggedInUser) => {
        try {
            const response = await axios.get(`http://54.163.10.39:5005/api/users/${loggedInUser.username}`,{
                headers: {
                    "Content-Type": "application/json",
