...
const TopManagersChart = ({ data = [] }) => {
  if (!data.length) {
    return <div className="p-6 bg-white rounded-lg shadow-lg text-center text-gray-700">No Manager Data Available</div>;
  }
        ...
        <div className="p-4 bg-gray-100 rounded-lg shadow-md">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 50 }}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="totalRoyalty" stroke="#10B981" name="Full Royalty" strokeWidth={3} dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
